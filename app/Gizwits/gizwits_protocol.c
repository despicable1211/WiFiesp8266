...
static int8_t gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagauto)
    {
        ...
        info->num++;
        dataPoints->valueauto = gizDecompressionValue(AUTO_BYTEOFFSET,AUTO_BITOFFSET,AUTO_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagfixed_point)
    {
        info->event[info->num] = EVENT_FIXED_POINT;
        info->num++;
        dataPoints->valuefixed_point = gizDecompressionValue(FIXED_POINT_BYTEOFFSET,FIXED_POINT_BITOFFSET,FIXED_POINT_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagedgewise)
    {
        info->event[info->num] = EVENT_EDGEWISE;
        info->num++;
        dataPoints->valueedgewise = gizDecompressionValue(EDGEWISE_BYTEOFFSET,EDGEWISE_BITOFFSET,EDGEWISE_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagseparate_room)
    {
        info->event[info->num] = EVENT_SEPARATE_ROOM;
        info->num++;
        dataPoints->valueseparate_room = gizDecompressionValue(SEPARATE_ROOM_BYTEOFFSET,SEPARATE_ROOM_BITOFFSET,SEPARATE_ROOM_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagelectricize)
    {
        info->event[info->num] = EVENT_ELECTRICIZE;
        info->num++;
        dataPoints->valueelectricize = gizDecompressionValue(ELECTRICIZE_BYTEOFFSET,ELECTRICIZE_BITOFFSET,ELECTRICIZE_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagforward)
    {
        info->event[info->num] = EVENT_FORWARD;
        info->num++;
        dataPoints->valueforward = gizDecompressionValue(FORWARD_BYTEOFFSET,FORWARD_BITOFFSET,FORWARD_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagbackward)
    {
        info->event[info->num] = EVENT_BACKWARD;
        info->num++;
        dataPoints->valuebackward = gizDecompressionValue(BACKWARD_BYTEOFFSET,BACKWARD_BITOFFSET,BACKWARD_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flaganticlockwise)
    {
        info->event[info->num] = EVENT_ANTICLOCKWISE;
        info->num++;
        dataPoints->valueanticlockwise = gizDecompressionValue(ANTICLOCKWISE_BYTEOFFSET,ANTICLOCKWISE_BITOFFSET,ANTICLOCKWISE_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }



    if(0x01 == issuedData->attrFlags.flagclockwise)
    {
        info->event[info->num] = EVENT_CLOCKWISE;
        info->num++;
        dataPoints->valueclockwise = gizDecompressionValue(CLOCKWISE_BYTEOFFSET,CLOCKWISE_BITOFFSET,CLOCKWISE_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
    ...
    if(0x01 == issuedData->attrFlags.flagpower)
    {
        ...
        info->num++;
        dataPoints->valuepower = gizDecompressionValue(POWER_BYTEOFFSET,POWER_BITOFFSET,POWER_LEN,(uint8_t *)&issuedData->attrVals.wBitBuf,sizeof(issuedData->attrVals.wBitBuf));
    }
...
static int8_t gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuefixed_point != cur->valuefixed_point)
    {
        GIZWITS_LOG("valuefixed_point Changed\n");
        ret = 1;
    }
    if(last->valueedgewise != cur->valueedgewise)
    {
        GIZWITS_LOG("valueedgewise Changed\n");
        ret = 1;
    }
    if(last->valueseparate_room != cur->valueseparate_room)
    {
        GIZWITS_LOG("valueseparate_room Changed\n");
        ret = 1;
    }
    if(last->valueelectricize != cur->valueelectricize)
    {
        GIZWITS_LOG("valueelectricize Changed\n");
        ret = 1;
    }
    if(last->valueforward != cur->valueforward)
    {
        GIZWITS_LOG("valueforward Changed\n");
        ret = 1;
    }
    if(last->valuebackward != cur->valuebackward)
    {
        GIZWITS_LOG("valuebackward Changed\n");
        ret = 1;
    }
    if(last->valueanticlockwise != cur->valueanticlockwise)
    {
        GIZWITS_LOG("valueanticlockwise Changed\n");
        ret = 1;
    }
    if(last->valueclockwise != cur->valueclockwise)
    {
        GIZWITS_LOG("valueclockwise Changed\n");
        ret = 1;
    }
...
static int8_t gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    gizCompressValue(AUTO_BYTEOFFSET,AUTO_BITOFFSET,AUTO_LEN,(uint8_t *)devStatusPtr,dataPoints->valueauto);
    gizCompressValue(FIXED_POINT_BYTEOFFSET,FIXED_POINT_BITOFFSET,FIXED_POINT_LEN,(uint8_t *)devStatusPtr,dataPoints->valuefixed_point);
    gizCompressValue(EDGEWISE_BYTEOFFSET,EDGEWISE_BITOFFSET,EDGEWISE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueedgewise);
    gizCompressValue(SEPARATE_ROOM_BYTEOFFSET,SEPARATE_ROOM_BITOFFSET,SEPARATE_ROOM_LEN,(uint8_t *)devStatusPtr,dataPoints->valueseparate_room);
    gizCompressValue(ELECTRICIZE_BYTEOFFSET,ELECTRICIZE_BITOFFSET,ELECTRICIZE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueelectricize);
    gizCompressValue(FORWARD_BYTEOFFSET,FORWARD_BITOFFSET,FORWARD_LEN,(uint8_t *)devStatusPtr,dataPoints->valueforward);
    gizCompressValue(BACKWARD_BYTEOFFSET,BACKWARD_BITOFFSET,BACKWARD_LEN,(uint8_t *)devStatusPtr,dataPoints->valuebackward);
    gizCompressValue(ANTICLOCKWISE_BYTEOFFSET,ANTICLOCKWISE_BITOFFSET,ANTICLOCKWISE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueanticlockwise);
    gizCompressValue(CLOCKWISE_BYTEOFFSET,CLOCKWISE_BITOFFSET,CLOCKWISE_LEN,(uint8_t *)devStatusPtr,dataPoints->valueclockwise);
    gizCompressValue(POWER_BYTEOFFSET,POWER_BITOFFSET,POWER_LEN,(uint8_t *)devStatusPtr,dataPoints->valuepower);
